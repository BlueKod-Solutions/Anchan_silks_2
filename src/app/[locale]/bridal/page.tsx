...
import { Sparkles, Gem, Crown, Shirt } from 'lucide-react';
  ...
  const benefits = [
    {
      ...
      desc: locale === 'kn'
        ? 'ಶಾಶ್ವತ ಪರಂಪರೆ ಮತ್ತು ಆಧುನಿಕ ವಿನ್ಯಾಸ'
        : 'Eternal heritage meets modern elegance',
    },
    {
      icon: Shirt,
      title: locale === 'kn' ? 'ಪ್ರಿಮಿಯಂ ರೇಷ್ಮೆ' : 'Premium Silks',
      desc: locale === 'kn'
        ? 'ಭಾರತದ ಅತ್ಯುತ್ತಮ ರೇಷ್ಮೆ ನೇಮಕರಣ'
        : 'Hand-selected pure silk varieties',
    },
    {
      icon: Crown,
      title: locale === 'kn' ? 'ಧಾರೀದಾರ ನೈಪುಣ್ಯ' : 'Expert Craftsmanship',
      desc: locale === 'kn'
        ? 'ತಜ್ಞ ಕಸೂತೀ ಕರ್ತೃತ್ವ ಮತ್ತು ವಿಸ್ತಾರ'
        : 'Intricate embroidery with perfection',
    },
    {
      icon: Gem,
      title: locale === 'kn' ? 'ಕಸ್ಟಮೈಜೇಶನ' : 'Customization',
      desc: locale === 'kn'
        ? 'ನಿಮ್ಮ ಅನನ್ಯ ದೃಷ್ಟಿಕೋನಕ್ಕೆ ಮೆರೆಯುತ್ತವೆ'
        : 'We create your dream bridal saree',
    },
  ];
      ...
      <main className="min-h-screen bg-cream-50 pt-20">
        ...
        <div className="relative h-screen md:h-[600px] overflow-hidden">
          ...
          <div className="absolute inset-0">
            ...
            {/* Base rich gradient */}
            <div className="absolute inset-0 bg-gradient-to-br from-[#1a0f0f]/90 via-[#3b1f1f]/70 to-[#5a2e2e]/50" />

            {/* Cinematic light focus */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,rgba(255,215,160,0.25),transparent_60%)]" />

            {/* Vignette */}
            <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-transparent to-black/40" />

            {/* Grain texture (add file in /public/textures/grain.png) */}
            <div className="absolute inset-0 opacity-[0.08] mix-blend-overlay pointer-events-none bg-[url('/textures/grain.png')]" />
          ...
          <div className="relative h-full flex items-center z-10">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-28 md:pt-20 lg:pt-16">
              <div className="max-w-2xl animate-[fadeUp_1s_ease-out]">

                {/* Premium Badge */}
                <div className="mb-6">
                  <span className="inline-block text-gold-300 text-[11px] tracking-[0.4em] uppercase border border-gold-500/30 px-4 py-1 bg-black/20 backdrop-blur-sm">
                    ✦ {t('bridal.badge')}
                  </span>
                </div>

                {/* Heading */}
                <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl text-white mb-6 leading-[1.1] tracking-tight drop-shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
                  {t('bridal.title')}
                </h1>

                {/* Divider */}
                <div className="w-16 h-[2px] bg-gradient-to-r from-gold-500 to-transparent mb-6" />

                {/* Subtitle */}
                <p className="text-cream-200 text-lg md:text-xl max-w-xl mb-8 leading-relaxed">
                  {t('bridal.subtitle')}
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-gold-500 to-gold-400 text-maroon-950 font-semibold shadow-[0_10px_30px_rgba(212,175,55,0.3)] hover:scale-[1.03] transition-all duration-300"
                  >
                  ...
                  <a
                    ...
                    className="inline-flex items-center justify-center px-8 py-4 border border-gold-500/40 text-gold-300 font-semibold backdrop-blur-md bg-white/5 hover:bg-gold-500/10 transition-all duration-300"
                  >
              ...
              {benefits.map((benefit, index) => (
                ...
                >
                  <div className="mb-4 flex justify-center">
                    <div className="w-12 h-12 flex items-center justify-center rounded-full bg-gold-500/10 group-hover:scale-110 transition-all duration-300">
                      <benefit.icon className="w-5 h-5 text-gold-600" />
                    </div>
                  </div>
                  <h3 className="font-serif text-xl text-maroon-900 mb-2 tracking-tight">{benefit.title}</h3>
                  <p className="text-cream-700 text-sm leading-relaxed">{benefit.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="relative rounded-lg overflow-hidden aspect-square animate-fade-up [animation-delay:200ms] shadow-2xl">
                <Image
                  src="/images/bridal/bridal-about1.png"
                  alt="Bridal Experience"
                  fill
                  className="object-cover contrast-105 brightness-105"
